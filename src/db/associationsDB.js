...
const businessUnitsDB = require('./businessUnitsDB')
const applicationsDB = require('./applicationsDB')
const userGroupsDB = require('./userGroupsDB')
const entityDB = require('./entityDB')
const deviceGroupsDB = require('./deviceGroupsDB')

const getTopLevelEntities = async() => {
    try{

        const result = []

        const query = `SELECT a.entity_id,a.entity_name, a.entity_type, b.entity_type as child_entity_type, COUNT(b.*)
...
const getAllAssociationsByEntityID = async(data) => {
    try{
        ...
        JOIN PUBLIC.getTree(can.id) t ON t.table_id = can.id 
		WHERE AND entity_id = $1 AND entity_name = $2 `,[ entityID, entityName ])
        return rows[0].json_agg
    } catch(e) {
        throw e
    }
